...
with st.sidebar:
    ...
    st.write("Selecione um período e uma ação ou índice.")
...
data_treino = metadados['data_treino'] #data final do DF utilizado no treino e teste do modelo
...
papel = Acao(ticker)
df_vis = papel.retorna_df(periodo, data_final) #monta o df para visualização
...
#-----------------------------------
tab1, tab2, tab3 = st.tabs(['Visualização', 'Previsão', 'Info'])
#-----------------------------------
...
with tab1:
    ...
    fig1.add_trace(go.Scatter(
        x=df_vis.index,  # x-axis
        y=df_vis['fechamento'],  # y-axis
...
with tab2:
    ...
    df_pred = df_pred.rename(columns={'AutoARIMA': 'fechamento_predito'})
    ...
    df_vis_tendencia = df_vis_tendencia.rename(columns={'ds': 'Data'})
    df_vis_tendencia['tendencia_real'] = df_vis_tendencia['tendencia_real'].apply(lambda x: 'Subiu' if x == 2 else('Igual' if x == 1 else 'Desceu'))
    df_vis_tendencia['tendencia_predita'] = df_vis_tendencia['tendencia_predita'].apply(lambda x: 'Subiu' if x == 2 else('Igual' if x == 1 else 'Desceu'))
    # FUNÇÃO QUE APLICA O ESTILO NAS CÉLULAS
    def estilo_previsao(row):
        if row["tendencia_predita"] == row["tendencia_real"]:
            cor = "background-color: #d4edda; color: #155724"
        else:
            cor = "background-color: #f8d7da; color: #721c24"

        return [
            cor if col == "tendencia_predita" else ""
